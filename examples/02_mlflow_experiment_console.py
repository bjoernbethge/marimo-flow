...
def _build_tracking_uri(root: str) -> str:
...
def _list_experiments(client: MlflowClient) -> pl.DataFrame:
...
def _runs_to_dataframe(runs: list[mlflow.entities.Run]) -> pl.DataFrame:
...
@app.cell
def _(reload_button, tracking_root):
    try:
        uri = _build_tracking_uri(tracking_root.value)
        mlflow.set_tracking_uri(uri)
        client = MlflowClient(tracking_uri=uri)
    except Exception as exc:  # pragma: no cover - shown in UI
        mo.md(f"⚠️ Unable to connect: `{exc}`")
        client = None
    return client
...
def _(client):
    ...
    experiments_df = _list_experiments(client)
    if experiments_df.is_empty():
        mo.md("_No experiments found._")
        return experiments_df, None
    ...
    return experiments_df, exp_dropdown


@app.cell
def _(client, exp_dropdown):
    if client is None or exp_dropdown is None or not exp_dropdown.value:
        return pl.DataFrame()
